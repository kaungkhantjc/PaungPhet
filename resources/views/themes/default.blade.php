...
<main class="w-full min-h-screen relative">
    ...
    {{-- Toast Notification --}}
    @if(session('success'))
        <div id="toast-wrapper" class="fixed top-18 left-0 w-full flex justify-center z-[100] pointer-events-none">
            <div id="toast-notification" class="pointer-events-auto bg-white/90 backdrop-blur-xl border border-pink-200 shadow-xl shadow-pink-100/50 rounded-full px-6 py-3 flex items-center gap-3 animate-toast-enter">
                <div class="bg-green-100 rounded-full p-1">
                    <x-heroicon-m-check class="w-4 h-4 text-green-600" />
                </div>
                <span class="text-pink-600 font-medium text-sm">{{ session('success') }}</span>
                <button onclick="closeToast()" class="ml-2 text-slate-400 hover:text-pink-500 transition-colors cursor-pointer">
    ...
    <div class="max-w-3xl mx-auto -mt-20 relative z-20 px-4 pb-20 space-y-10">
        ...
        @if($guest && $guest->is_notable)
            {{-- Logic Refactored: Removed the outer check that hid the form/note display --}}
